...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        return scheme_apply(self, operands.map(lambda variable: scheme_eval(variable, env)), env)
        # END PROBLEM 5
...
def do_and_form(expressions, env):
    ...
    if len(expressions) == 0:
        ...
        if scheme_truep(first):
            return first
        return False
    while scheme_truep(scheme_eval(expressions.first, env)):
...
def do_or_form(expressions, env):
    ...
    if len(expressions) == 0:
        ...
        return scheme_eval(expressions.first, env, True) 
    while scheme_falsep(scheme_eval(expressions.first, env)):
...
def make_let_frame(bindings, env):
    ...
    while check_form(bindings.first, min = 1, max = 2):
        bingdings = bindings.second

    formals = Pair(bindings.first.first, nil)
    vals = Pair(scheme_eval(bindings.first.second.first, env), nil)

    while bindings.second is not nil:
        formals = Pair(bindings.second.first.first, formals)
        vals = Pair(scheme_eval(bindings.second.first.second.first, env), vals)
